...
sed -i "/if (List.includes(location.host)) {} else {let support = document.createElement('iframe');/,/}/d" release/ShortLink1-modified.user.js
...
sed -i "/if (List.includes(location.host)) {} else {let support = document.createElement('iframe');/,/}/d" release/ShortLink3-modified.user.js
